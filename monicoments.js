...
api.get("repos/seita-ifce/hello-js-v3/issues/2/comments").then(ret => {
    ...
    //percorre o array de usuarios, verifica se o login é sombriks e se a data é menor que 2017-09-14, utilizando template string.
    for( let i = 0; i < users.length; i++){ 
        let dataComentario = new Date(users[i].created_at)
        if (users[i].user.login != "sombriks" && dataComentario < (new Date("2017-09-14"))) {
